...
| Canvas | LiteGraph original navigation: empty-canvas drag pans; Cmd/Ctrl+drag selects a rectangle; Shift-click adds selection; Space+drag or middle-drag also pans; wheel zooms. Drag a slot to link. Cards show title, state pill, live line, type and age, drawn by the node adapter from the daemon's node. Clicking a card selects it; double-clicking its header edits its name. Right-click menus and the search box are off until their items send daemon commands (phase 1 ruling 2). |
...
- **Workflow selection and clipboard.** Empty-canvas dragging pans, and Cmd/Ctrl+drag selects a
  rectangle using LiteGraph's native selection, with additive gestures intact.
  Cmd/Ctrl+A/C/V selects all, copies, or pastes while the canvas has focus. Copy places GraphCode workflow
  ...
  types refuse the entire paste or workflow load before mutation. Clipboard events use the browser's
  normal path without background reads or cached fallbacks; a paste during a canvas transition is
  refused rather than modifying the previous canvas. Text fields retain normal selection and
  clipboard behavior. There are no dedicated selection/clipboard buttons or persistent instruction
  strips. Folder details are available by hovering over Run or a saved workflow in the sidebar.
