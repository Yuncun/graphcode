    ...
    <div class="body">
      ...
      <section class="center">
        <div v-if="activeGraph" class="toolbar" data-testid="toolbar">
          <button class="primary" data-testid="run" :disabled="!startable || nodeTypesLoading || !!bindingDocument || savingRun" :title="`${activeDocument ? (activeDocument.project ?? 'Choose a folder before running') + '\n' : ''}${counts.drafts} draft card(s) and ${counts.wires} draft wire(s)`" @click="startCards()">
            Run<span v-if="startable" class="count" data-testid="run-count">{{ counts.drafts + counts.wires }}</span>
          </button>
          <button data-testid="save-workflow" @click="saveWorkflow">Save template…</button>
        </div>
        <GraphCanvas v-if="activeGraph" ref="canvasView" :graph="activeGraph" :node-types="nodeTypes" :types-loading="nodeTypesLoading" @select="selected = $event" @rename="onRename" @delete-live="onDeleteLive" @delete-edge="onDeleteEdge" @document-changed="counts = $event" @problem="store.pushError($event)" />
        <div v-else class="empty" data-testid="empty">{{ activeDocument?.project ? "Waiting for project…" : active && documentID(active) ? "Loading workflow…" : "No open workflows" }}</div>
      </section>
    </div>
