...
- [ ] **Step 0: Controller ruling after Task 7: keep the fitted graph readable**

Task 7's sidebar and inspector leave a 1400 px window with an 860 px canvas, and the fit for the nine-card fixture landed at scale 0.5, below litegraph's `low_quality_zoom_threshold` (default 0.6), where it stops drawing titles, badges and slot labels. Three small changes:

1. `web/app/canvas/viewport.ts`: add `export const FIT_MIN_SCALE = 0.6;` and, in `fitToNodes`, after the `> 1` clamp, clamp the other way:
...
In `web/tests/viewport.test.ts`, import `FIT_MIN_SCALE` and replace the test "zooms out to show a large graph in one call" with:
...
2. `web/app/canvas/GraphCanvas.vue`, right after the `LGraphCanvas` is constructed: `canvas.low_quality_zoom_threshold = 0.5;` with the comment "litegraph stops drawing text below this scale (its default is 0.6); the fit floor is 0.6, so text survives the fit and one zoom step out".

3. `web/playwright.config.ts`: `viewport: { width: 1600, height: 1000 }`, so the matrix screenshots show a laptop-sized window rather than a cramped one. Rerun the phase 0 matrix after this step and look at `P0-02-two-tabs-all-states.png`: titles, badges and slot labels must be drawn.
